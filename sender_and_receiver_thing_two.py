...
def listen(publisher_thread): # The listen() method takes the publisher thread as a parameter
    ...
    for dweet in dweepy.listen_for_dweets_from(config.ThingOneName): # For loop listens for dweets from a specific thing called TestThingOne
...
def publish(): # The publish() method takes no parameters
    ...
    while publisher_state: # While publisher state is true execute the following code
        ...
        result = dweepy.dweet_for(config.ThingTwoName, {"Temperature": temperature, "Humidity": humidity}) # Send a dweet from a specific thing called TestThingTwo with a key of TestNum and value of the variable called num and store it in a variable called result
